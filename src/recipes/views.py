...
def update_recipe(request, pk):
    ...
    if request.user == recipe.author:
        ...
        return render(request, "recipes/update_recipe.html", {'form': form, 'recipe': recipe, 'ingredients': current_ingredients})
    else:
...
class RecipesListView(ListView):
    ...
    def get_queryset(self):
        ...
        if recipe_name:
            combined_query &= Q(name__icontains=recipe_name)
    ...
    def get_context_data(self, **kwargs):
        ...
        if "chart_type" in self.request.GET:
            ...
            data = pd.DataFrame.from_records(queryset.values('name', 'cooking_time'))

            chart_data = {"name": data.get("name", []), "cooking_time": data.get("cooking_time", [])}

            if chart_type == "#1":
                chart_data["labels"] = chart_data.get("name")
            elif chart_type == "#2":
                chart_data["labels"] = chart_data.get("name")
            else:
